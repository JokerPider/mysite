...
from models import UserModel,ShareModel,AnswerModel
...
def index():
    context = {
        'questions': ShareModel.query.order_by('-create_time').all()
    }
...
def question():
    if flask.request.method == 'GET':
        ...
        type = flask.request.form.get('inlineRadioOptions')
        question_model = ShareModel(title=title,content=content, type=type)
...
@app.route('/d/<id>/')
def detail(id):
    question_model = ShareModel.query.get(id)
...
def comment():
    ...
    answer_model.question = ShareModel.query.get(question_id)
...
def search():
    ...
    questions = ShareModel.query.filter(or_(ShareModel.title.contains(q), ShareModel.content.contains(q), ShareModel.type.contains(q))).order_by('-create_time')
    context = {
        'questions': questions
    }
    return flask.render_template('index.html',**context)
